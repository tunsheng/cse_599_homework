...
class ConvLayer(Layer):
    ...
    def im2col_helper(x_padded, filter_height, filter_width, padding, stride):
        nimages, input_channels, padded_height, padded_width = x_padded.shape
        output_h = int((padded_height-filter_height) // stride) + 1
        output_w = int((padded_width-filter_width) // stride) + 1
        ...
        for n in prange(nimages):
            for i in prange(output_h):
               for j in prange(output_w):
                   ...
                   start_w = j*stride
                   end_w = start_w+filter_width
                   patch = x_padded[n, :, start_h:end_h, start_w:end_w].copy()
                   col[n, :, j+i*output_w] = patch.flatten()
        return col
    ...
    def im2col(self, x, filter_height, filter_width, padding, stride):
        p = padding
        nimages, input_channels, padded_height, padded_width = x.shape

        padded = np.pad(x, ((0, 0), (0, 0), (p, p), (p, p)), mode='constant')
        col = self.im2col_helper(padded, filter_height, filter_width, padding, stride)
        col = col.transpose(1 , 2, 0).reshape(filter_height*filter_width*input_channels, -1)
        return col
